...
from backend.btc.core.signals import scan_for_signals, TradingSignal
from backend.btc.data.btc_markets import fetch_active_btc_markets, BtcMarket
from backend.btc.data.crypto import fetch_crypto_price, compute_btc_microstructure
...
async def startup():
    ...
    from backend.btc.core.btc_scheduler import start_scheduler, log_event
    start_scheduler()
...
async def shutdown():
    from backend.btc.core.btc_scheduler import stop_scheduler
    stop_scheduler()
...
async def simulate_trade(signal_ticker: str, db: Session = Depends(get_db)):
    from backend.btc.core.btc_scheduler import log_event
...
async def run_scan(db: Session = Depends(get_db)):
    from backend.btc.core.btc_scheduler import run_manual_scan, log_event
    ...
    if settings.WEATHER_ENABLED:
        try:
            from backend.weather.core.weather_signals import scan_for_weather_signals
...
async def settle_trades_endpoint(db: Session = Depends(get_db)):
    from backend.common.settlement import settle_pending_trades, update_bot_state_with_settlements
    from backend.btc.core.btc_scheduler import log_event
...
async def get_kalshi_status():
    ...
    from backend.common.data.kalshi_client import KalshiClient, kalshi_credentials_present
...
async def get_weather_forecasts():
    ...
    try:
        from backend.weather.data.weather import fetch_ensemble_forecast, CITY_CONFIG
...
async def get_weather_markets():
    ...
    try:
        from backend.weather.data.weather_markets import fetch_polymarket_weather_markets
        ...
        if settings.KALSHI_ENABLED:
            try:
                from backend.common.data.kalshi_client import kalshi_credentials_present
                from backend.common.data.kalshi_markets import fetch_kalshi_weather_markets
...
async def get_events(limit: int = 50):
    from backend.btc.core.btc_scheduler import get_recent_events
...
async def start_bot(db: Session = Depends(get_db)):
    from backend.btc.core.btc_scheduler import start_scheduler, log_event, is_scheduler_running
