---
source_url: https://x.com/chongdashu
source_type: x-post
ingested: 2026-04-23
sha256: a214ace69d32e340
author: Chong-U @chongdashu
time: 14h
github_repos: ["stanford-iris-"]
urls: []
---

Article
GPT Images 2.0 For Game Sprites? Here's how.
OpenAI just dropped their new Images 2.0 model and the results are genuinely stunning.
But the questions every game dev is asking:
can it  produce usable pixel art sprites?
what about transparency?...
29
96
1K
104K
ₕₐₘₚₜₒₙ
@hamptonism
·
8h
nah what the f*ck are these qaunts in china doing.
0:03 / 0:30
From
Marry Evan
86
96
1.5K
215K
Bill The Investor
@billtheinvestor
·
12h
Translated from Chinese
Show original
This open-source missile tracker is absolutely insane!!
3
70
504
67K
Jake (softservo)
@soft_servo
·
3h
Vibe coding a robot with GPT 5.5!
This is a URDF of a 7dof robot arm with functional kinematics, a custom gui, and STEP parts/assembly, 100% generated in Codex (minus the gripper).
A similar result would have taken me weeks stitching half a dozen tools together. Insane stuff.
0:03 / 0:23
22
81
806
39K
Raanan Tzory
@RaananTz
Been vibe coding something small:
- Studied 2,000+ tech brand logos
- Turned patterns into ready-to-use logos
- All human-designed
- $69 for a full brand kit
Show more
Professional logo and brand kit $69
From anymark.co
1
24K
Riley Brown
@rileybrown
·
3h
GPT 5.5 Experiment:
Threejs: 25 trains, hills, switch views, change speeds, crashes, Crash Counter.
Just a few prompts...
idea from
@petergostev
he did something similar.
0:03 / 1:10
14
19
241
17K
Emily Han
@emilyhanyf
·
5h
built an interactive guide to teach you the basics of mahjong !
it includes rulesets from different regions (hk and taiwan for now) with a few interactive elements to illustrate mahjong concepts. more demos in thread, and a bit about the tools i used to build this.
the one
Show more
0:41
60
344
3.2K
129K
Tim Jayas
@TimJayas
·
8h
BREAKING: Never pay for clipping tools again!
Someone built fully AI powered tool that just destroyed the $49/month clipping industry
> Clip any YouTube or local videos
> No watermarks, no limits, self-hosted
> Can even plug in Claude or any AI model
100% free & open
Show more
29
44
686
52K
Michel Lieben
@MichLieben
·
12h
I'm giving away the Claude Code skills we use to manage $300k/mo in ad spend at ColdIQ.
4X ROAS on $1M+ spent.
Ivan, our head of growth, built them off 300+ hours running ad campaigns for our clients. They run Google, Meta, and LinkedIn ads from the terminal in plain English:
Show more
1.9K
150
1.5K
140K
Sourcebot. (YC F25)
@sourcebot_dev
Give agents context across your entire codebase. Open source, 1 min install.
docs
From sourcebot.dev
176
682
3.9M
Eric
@Ex0byt
·
10h
We now have amazing local models (and more will come). We need better harnesses. Stop listening to the blue-pill shills. Read the paper and audit your own harnesses.
Paper:
http://
arxiv.org/abs/2603.28052
Code:
http://
github.com/stanford-iris-
lab/meta-harness
…
(Credit: Stanford CS faculty/Iris Lab)
2
4
91
5K
Nathan Wilbanks
@NathanWilbanks_
·
10h
Hell yea!! Harness meta optimization is the way. I implemented a self optimization system awhile back in my harness that learns from every trace. Agents, skills, tools, and workflows all self improve with the built in eval / insight system. The quality just keeps getting better
2
13
605
Alex Finn
@AlexFinn
·
8h
There is a Mythos level AI model dropping later today that will be available to every person in the world day 1
Do you understand what is about to happen?
Democratized super intelligence
Anyone can create any future they want
147
56
988
122K
Paul Young
@PaulYoungX
·
18h
Translated from French
Show original
This 20-year-old guy, an American student, just pulled off a feat that entire teams failed to achieve in two years.
One night. Three dollars in components. An infrared diode, a photoresistor, a microcontroller the size of a coin, an old laptop.
Claude Code writes the code
Show more
Quote
Defileo
@defileo
·
Apr 18
Article
Don't be lazy to automate your work with routines
Most people think Claude is something you talk to, tho it is now something you deploy.
Anthropic quietly shipped something that changes how you think about AI entirely, it is called Routines.
And...
24
173
1.7K
501K
